...
namespace MSBuildTasks
{
    public class SimpleTask : ContextIsolatedTask
    {
        ...
        protected override bool ExecuteIsolated()
        {
            ...
            //var x = new RazorEngineCore.Writers.HtmlWriter();
            //var j = new RazorEngineCore.Writers.JsonWriter();

            var dl = new DirectoryLoader(new DirectoryInfo(this.TargetDir));
            
            //var assemblyNameX = AssemblyLoadContext.GetAssemblyName(outputFile);
            //var assembly = Assembly.Load(assemblyNameX);
            ...
            var ass = AssemblyLoadContext.Default.LoadFromAssemblyPath(outputFile);
            
            var attrs = ass.GetCustomAttributesData();

            foreach (var attributeData in attrs)
            {
                Log.LogError(attributeData.ToString());
            }
            ...
            var template = razorEngine.Compile<RazorEngineCorePageModel>("Ello");
            ...
            foreach (var assemblyName in Assembly.GetExecutingAssembly().GetReferencedAssemblies())
            {
                ...
                Log.LogWarning(assemblyName.FullName);
            }
    ...
    public class DirectoryLoader : AssemblyLoadContext
    {
    ...
    partial class ContextIsolatedTask : Task
    {
        ...
        protected class CustomAssemblyLoader : AssemblyLoadContext
        {
